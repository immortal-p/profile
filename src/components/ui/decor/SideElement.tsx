...
const SideElement = ({ children, side = 'right', top = 0, left, right, hideBelow }: SideElementProps) => {
    ...
    return (
        <motion.div
            initial={prefersReducedMotion ? false : { opacity: 0, x: side === 'right' ? 100 : -100 }}
            whileInView={prefersReducedMotion ? undefined : { opacity: 1, x: side === 'right' ? 55 : -55 }}
            transition={{

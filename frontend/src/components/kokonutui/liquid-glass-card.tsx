...
const GLASS_SHADOW_DARK =
    'dark:shadow-[0_0_8px_rgba(0,0,0,0.03),0_2px_6px_rgba(0,0,0,0.08),inset_3px_3px_0.5px_-3.5px_rgba(255,255,255,0.09),inset_-3px_-3px_0.5px_-3.5px_rgba(255,255,255,0.85),inset_0px_1px_1px_-0.5px_rgba(255,255,255,0.6),inset_-1px_-1px_1px_-0.5px_rgba(255,255,255,0.6),inset_0_0_12px_0px_rgba(255,255,255,0.06),0_0_12px_rgba(0,0,0,0.15)]'
...
const VolumeBars = React.memo(({ isPlaying }: VolumeBarsProps) => {
    ...
    return (
        <div className="pointer-events-none flex h-8 w-10 items-end gap-0.5">
            {bars.map((bar) => (
                <div
                    className={cn('w-0.75 rounded-sm', isPlaying && 'animate-bounce-music')}
                    key={bar.id}
                    style={{
...
const ProgressBar = React.memo(({ currentTime, totalDuration, onSeek }: ProgressBarProps) => {
    ...
    return (
        <>
            ...
            >
                <div
                    className="h-full bg-linear-to-r from-[#FF2E55] to-[#FF6B88] transition-all duration-200"
...
export function NotificationCenter() {
    ...
    return (
        <div className="w-full max-w-sm">
            <LiquidGlassCard className="gap-3.5 rounded-3xl border border-zinc-200/60 bg-linear-to-br from-zinc-50 to-zinc-100 p-4 shadow-xl dark:border-zinc-700/60 dark:from-zinc-900 dark:to-black">
                <div className="flex items-center gap-3">
                    <div className="relative mr-2 mb-4 h-16 w-16 shrink-0 overflow-hidden rounded-2xl bg-linear-to-br from-pink-400 via-pink-300 to-rose-200 shadow-lg ring-1 ring-black/5 dark:shadow-xl">
                        <img

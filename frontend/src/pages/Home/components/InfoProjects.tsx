                    ...
                    <LiquidGlassCard className="rounded-md bg-transparent shadow-none border-none p-1 *:shadow-0 z-20">
                        ...
                        <div className="p-2 flex flex-wrap gap-2 border-t border-b border-(--text-cl) text-[16px] text-(--text-cl) relative">
                            {project.stack.map((sk, index) => (
                                <span key={index} className="border border-(text-cl) rounded-sm px-2 py-1 whitespace-nowrap inline-block">{sk}</span>
                            ))}
                        </div>
                            ...
                            {project.live && (
                                <Button
                                    className="border rounded-sm border-(--icon-cl) text-(--white-cl) mr-4 mb-0">
                                        <Link to={project.liveUrl !== undefined ? project.liveUrl : ""} className="py-2 px-4">{project.live}</Link>
                                </Button> 
                            )}
                            <Button
